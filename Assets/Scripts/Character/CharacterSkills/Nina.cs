public class Nina : CharacterBase
{
    ...
    public override void ActiveSkill1(CharaController user)
    {
        ...
        targets.ForEach(target => totalDamage += SkillManager.Attack(user, target, user.Status.attackPower, 290, AttackPattern.Simultaneous));
    ...
    public override void ActiveSkill2(CharaController user)
    {
        ...
        targets.ForEach(target => SkillManager.Attack(user, target, user.Status.attackPower, 420, AttackPattern.Simultaneous));
